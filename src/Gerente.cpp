...
void Gerente::abrirContaCorrente(const std::string& cpf, double saldoInicial, double limiteChequeEspecial) {
    ...
    if (it != clientes.end()) {
        ...
        if (!novaConta->getTransacoes().empty()) {
            ...
            std::cout << "Conta Corrente criada, mas sem transa??es registradas.\n";
        }

        std::cout << "Finalizando abertura de conta corrente. Fluxo continua.\n";
    } else {
        std::cout << "Cliente n?o encontrado.\n";
    }
}

/**
 * @brief Abre uma nova conta poupan?a para um cliente existente.
 *
...
void Gerente::abrirContaPoupanca(const std::string& cpf, double saldoInicial, double taxaJuros) {
    ...
    if (it != clientes.end()) {
        ...
        if (!novaConta->getTransacoes().empty()) {
            std::cout << "Conta Poupan?a criada com sucesso na data " << novaConta->getTransacoes().back().getDataString() << "!\n";
        } else {
            std::cout << "Conta Poupan?a criada, mas sem transa??es registradas.\n";
        }

        std::cout << "Finalizando abertura de conta poupan?a. Fluxo continua.\n";
    } else {
        std::cout << "Cliente n?o encontrado.\n";
    }
}
...
/**
 * @brief Permite que o gerente realize v?rias opera??es como gerenciar clientes e contas.
 */
void Gerente::operarComoGerente() {
    ...
    do {
        ...
        std::cout << "3. Abrir Conta Poupan?a\n";
        ...
        try {
            switch (opcao) {
                case 1: {
                    ...
                    std::cout << "Digite o n?mero de telefone do cliente: ";
                ...
                case 4: {
                    int numeroConta;
                    std::cout << "Digite o n?mero da conta para tornar inativa: ";
                    std::cin >> numeroConta;
                ...
                case 5: {
                    std::string cpf;
                    std::cout << "Digite o CPF do cliente a ser exclu?do: ";
                    std::cin >> cpf;
                    excluirCliente(cpf);
                    break;
                }
...
/**
 * @brief Torna uma conta inativa, impedindo novas opera??es.
 *
...
void Gerente::tornarContaInativa(int numeroConta) {
    ...
    std::cout << "Conta " << numeroConta << " n?o encontrada.\n";
}

/**
 * @brief Exclui um cliente do sistema, desde que ele n?o tenha contas ativas.
 *
...
void Gerente::excluirCliente(const std::string& cpf) {
    ...
    if (it != clientes.end()) {
        if (it->second->getContas().empty()) {
            clientes.erase(it);
            std::cout << "Cliente com CPF " << cpf << " foi exclu?do.\n";
        } else {
            std::cout << "Cliente com CPF " << cpf << " possui contas ativas e n?o pode ser exclu?do.\n";
        }
    } else {
        std::cout << "Cliente com CPF " << cpf << " n?o encontrado.\n";
    }
}
...
void Gerente::editarDadosCliente(const std::string& cpf) {
    ...
    if (it != clientes.end()) {
        ...
        std::cout << "2. Editar Endere?o\n";
        std::cout << "3. Editar N?mero de Telefone\n";
        ...
        switch (opcao) {
            case 1:
                ...
                break;
            case 2:
                std::cout << "Digite o novo endere?o: ";
                std::cin.ignore();
                std::getline(std::cin, novoValor);
                cliente->setEndereco(novoValor);
                break;
            case 3:
                std::cout << "Digite o novo n?mero de telefone: ";
                std::cin >> novoValor;
